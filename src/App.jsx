...
export default function App(){ const [page,setPage]=useState('home'); const [category,setCategory]=useState(''); const [legalSection,setLegalSection]=useState('terms'); const [product,setProduct]=useState(null); const navigate=(next,data)=>{setPage(next); if(next==='catalog'||next==='world')setCategory(data||''); if(next==='legal')setLegalSection(data||'terms'); setProduct(null); window.scrollTo(0,0)}; const openProduct=p=>{setProduct(p);setPage('product');window.scrollTo(0,0)}; useEffect(()=>{const onPop=()=>navigate('home');window.addEventListener('popstate',onPop);return()=>window.removeEventListener('popstate',onPop)},[]); return <><Header navigate={navigate} onSearch={()=>navigate('catalog')}/>{page==='home'&&<Home navigate={navigate} openProduct={openProduct}/>} {page==='catalog'&&<Catalog category={category} openProduct={openProduct} navigate={navigate}/>} {page==='worlds'&&<Worlds navigate={navigate}/>} {page==='world'&&<WorldPage worldId={category} navigate={navigate}/>} {page==='product'&&product&&<ProductDetail product={product} navigate={navigate}/>} {page==='how'&&<How navigate={navigate}/>} {page==='about'&&<About navigate={navigate}/>} {page==='contact'&&<About contact navigate={navigate}/>} {page==='legal'&&<Legal section={legalSection} navigate={navigate}/>}<Footer navigate={navigate}/></> }
