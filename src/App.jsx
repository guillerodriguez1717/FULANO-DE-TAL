...
function Home({ navigate, openProduct }) {
  ...
  return <main>
    ...
    <section className="section products-section catalog-coming"><div className="contextual-section-title"><SectionTitle eyebrow="La vidriera de Fulano" title="Productos elegidos, con las cosas claras" text="Acá vas a encontrar muebles y otras cosas útiles con precio, medidas, plazo, entrega y condiciones bien explicadas."/><div className="mini-fulano-helper mini-search"><span><b>Fulano está pispeando</b><small>Mejor pocas cosas buenas que llenar por llenar.</small></span></div></div><div className="catalog-coming-paper"><b>Fulano dice:</b><span>“Antes de llenar la vidriera por llenar, mejor poner cosas que valgan la pena. Despacio y con buena letra.”</span><button className="secondary" onClick={() => navigate('catalog')}>Recorrer categorías</button></div></section>

    <PaymentMethods />

    <section className="order-section order-section-fulano"><div className="order-intro"><span className="eyebrow">Cómo comprar</span><h2>Elegís tranquilo.<br/>Fulano hace el resto.</h2><p>Muchos productos trabajan bajo pedido. Por eso, antes de poner un peso, confirmamos con vos precio, disponibilidad, plazo, pago y entrega.</p><div className="order-promise"><b>Primero las cosas claras.</b><span>Después sí, le damos para adelante.</span></div><button className="secondary light" onClick={() => navigate('how')}>Ver el proceso completo</button></div><div className="order-steps order-steps-flow">{[['01','Elegís','Encontrás lo que necesitás o nos contás qué estás buscando.'],['02','Consultás','Revisamos producto, medidas, variantes y cualquier duda.'],['03','Confirmamos','Precio, disponibilidad, plazo, forma de pago y entrega.'],['04','Lo pedimos','Con tu confirmación, encargamos el producto especialmente para vos.'],['05','Te acompañamos','Seguimos el pedido y coordinamos con vos hasta la entrega.']].map(s => <div className="order-step" key={s[0]}><span>{s[0]}</span><div><h3>{s[1]}</h3><p>{s[2]}</p></div></div>)}</div></section>
