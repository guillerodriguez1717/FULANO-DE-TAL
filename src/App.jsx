...
function Home({ navigate, openProduct }) {
  ...
  return <main>
    ...
    <section className="section products-section catalog-coming"><div className="contextual-section-title"><SectionTitle eyebrow="La vidriera de Fulano" title="Productos elegidos, con las cosas claras" text="Acá vas a encontrar muebles y otras cosas útiles con precio, medidas, plazo, entrega y condiciones bien explicadas."/><div className="mini-fulano-helper mini-search"><span className="mini-fulano-photo" aria-hidden="true"><img src="file_00000000da08820ea26b112407de4c25.png" alt=""/></span><span><b>Fulano está pispeando</b><small>Mejor pocas cosas buenas que llenar por llenar.</small></span></div></div><div className="catalog-coming-paper"><b>Fulano dice:</b><span>“Antes de llenar la vidriera por llenar, mejor poner cosas que valgan la pena. Despacio y con buena letra.”</span><button className="secondary" onClick={() => navigate('catalog')}>Recorrer categorías</button></div></section>

    <PaymentMethods />

    <section className="order-section"><div className="order-fulano-corner order-real-fulano" aria-hidden="true"><img src="file_00000000da08820ea26b112407de4c25.png" alt=""/></div><div className="order-intro"><span className="eyebrow">Bajo pedido, bien acompañado</span><h2>Vos elegís.<br/>Fulano se mueve.</h2><p>Antes de confirmar, revisamos juntos disponibilidad, plazo, envío y forma de pago. Claro desde el principio.</p><button className="secondary light" onClick={() => navigate('how')}>Conocé cómo funciona</button></div><div className="order-steps">{[['01','Elegís','El producto y las variantes que mejor van con tu casa.'],['02','Confirmamos','Disponibilidad, precio, plazo y entrega. Sin sorpresas.'],['03','Lo pedimos','Hacemos el pedido especialmente para vos.'],['04','Te acompañamos','Te contamos cómo avanza hasta que llega a casa.']].map(s => <div className="order-step" key={s[0]}><span>{s[0]}</span><div><h3>{s[1]}</h3><p>{s[2]}</p></div></div>)}</div></section>
